import sys
import os
import cv2
import numpy as np
from PyQt5.QtWidgets import QApplication, QMainWindow, QLabel, QVBoxLayout, \
    QHBoxLayout, QPushButton, QFileDialog, QDialog, QInputDialog, QScrollArea, QSizePolicy, QMessageBox
from PyQt5.QtGui import QPixmap, QImage, QFont
from PyQt5.QtCore import Qt

class ImageProcessorApp(QMainWindow):
    def __init__(self):
        super().__init__()
        self.setWindowTitle("Обработчик Изображений")
        self.setGeometry(100, 100, 800, 600)

        self.image_label = QLabel()
        self.image_label.setAlignment(Qt.AlignCenter)

        self.scroll_area = QScrollArea()
        self.scroll_area.setWidget(self.image_label)
        self.scroll_area.setWidgetResizable(True)

        self.create_photo_msg_box = QMessageBox()
        self.create_photo_msg_box.setIcon(QMessageBox.Information)
        self.create_photo_msg_box.setText('Загрузка веб-камеры, подождите.')
        self.create_photo_msg_box.setWindowTitle('Создание')
        self.create_photo_msg_box.setStandardButtons(QMessageBox.NoButton)

        self.warning_msg_box = QMessageBox()
        self.warning_msg_box.setIcon(QMessageBox.Warning)
        self.warning_msg_box.setText('Проверьте работу веб-камеры и повторите еще раз.')
        self.warning_msg_box.setWindowTitle('Ошибка')

        self.information_msg_box = QMessageBox()
        self.information_msg_box.setIcon(QMessageBox.Information)
        self.information_msg_box.setText('Фото создано!')
        self.information_msg_box.setWindowTitle('Информация')
        self.load_button = QPushButton("Загрузить изображение")
        self.load_button.clicked.connect(self.load_image)
        self.load_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.web_button = QPushButton("Сделать изображение с помощье веб-камеры")
        self.web_button.clicked.connect(self.create_photo_msg_box.show)
        self.web_button.clicked.connect(self.web_image)
        self.web_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.show_button = QPushButton("Показать оригинальное изображение")
        self.show_button.clicked.connect(self.show_image)
        self.show_button.setEnabled(False)
        self.show_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.channel_button = QPushButton("Показать канал изображения")
        self.channel_button.clicked.connect(self.choose_channel)
        self.channel_button.setEnabled(False)
        self.channel_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.rotate_button = QPushButton("Повернуть изображение")
        self.rotate_button.clicked.connect(self.rotate_image)
        self.rotate_button.setEnabled(False)
        self.rotate_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.negative_button = QPushButton("Негативное изображение")
        self.negative_button.clicked.connect(self.negative_image)
        self.negative_button.setEnabled(False)
        self.negative_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)

        self.circle_button = QPushButton("Нарисовать круг")
        self.circle_button.clicked.connect(self.draw_circle)
        self.circle_button.setEnabled(False)
        self.circle_button.setSizePolicy(QSizePolicy.Preferred, QSizePolicy.Expanding)


        button_layout = QVBoxLayout()
        button_layout.addWidget(self.load_button)
        button_layout.addWidget(self.web_button)
        button_layout.addWidget(self.show_button)
        button_layout.addWidget(self.channel_button)
        button_layout.addWidget(self.rotate_button)
        button_layout.addWidget(self.negative_button)
        button_layout.addWidget(self.circle_button)

        main_layout = QHBoxLayout()
        main_layout.addLayout(button_layout)
        main_layout.addWidget(self.scroll_area)

        central_widget = QDialog()
        central_widget.setLayout(main_layout)
        self.setCentralWidget(central_widget)

        self.image = None

    def web_image(self):
        cap = cv2.VideoCapture(0)
        if not cap.isOpened():
            self.warning_msg_box.exec()
            return

        while True:
            ret, frame = cap.read()
            if not ret:
                self.create_photo_msg_box.accept()
                self.warning_msg_box.exec()
                break
            self.create_photo_msg_box.accept()
            cv2.imshow("ПРОБЕЛ - чтобы сделать фото; ESCAPE - чтобы выйти из окна", frame)
            key = cv2.waitKey(1)
            if key == 27:
                cv2.destroyAllWindows()
                break
            elif key == 32:
                self.image = frame
                self.show_button.setEnabled(True)
                self.channel_button.setEnabled(True)
                self.rotate_button.setEnabled(True)
                self.negative_button.setEnabled(True)
                self.circle_button.setEnabled(True)
                self.display_image(self.image)
                self.information_msg_box.exec()
                cv2.destroyAllWindows()
                break
    def load_image(self):
        options = QFileDialog.Options()
        file_path, _ = QFileDialog.getOpenFileName(self,"Загрузить изображение", "", "Image Files (*.jpg *.jpeg *.png)", options=options)

        if file_path:
            # Ensure the file path is handled correctly
            file_path = os.path.normpath(file_path)
            self.image = cv2.imdecode(np.fromfile(file_path, dtype=np.uint8), cv2.IMREAD_COLOR)
            if self.image is not None:
                self.show_button.setEnabled(True)
                self.channel_button.setEnabled(True)
                self.rotate_button.setEnabled(True)
                self.negative_button.setEnabled(True)
                self.circle_button.setEnabled(True)

    def show_image(self):
        if self.image is not None:
            self.display_image(self.image)

    def choose_channel(self):
        if self.image is not None:
            channels = ["Синий", "Зеленый", "Красный"]
            channel, ok = QInputDialog.getItem(self,"Выбрать канал", "Канал:", channels, 0, False)
            if ok:
                if channel == "Синий":
                    img_blue = self.image.copy()
                    img_blue[:, :, 1] = 0
                    img_blue[:, :, 2] = 0
                    self.display_image(img_blue)
                elif channel == "Зеленый":
                    img_green = self.image.copy()
                    img_green[:, :, 0] = 0
                    img_green[:, :, 2] = 0
                    self.display_image(img_green)
                elif channel == "Красный":
                    img_red = self.image.copy()
                    img_red[:, :, 0] = 0
                    img_red[:, :, 1] = 0
                    self.display_image(img_red)

    def rotate_image(self):
        if self.image is not None:
            angle, ok = QInputDialog.getInt(self, "Повернуть изображение", "Угол:", 0, -360, 360, 1)
            if ok:
                height, width = self.image.shape[:2]
                center = (width / 2, height / 2)
                M = cv2.getRotationMatrix2D(center, angle, 1)
                rotated_img = cv2.warpAffine(self.image, M, (width, height))
                self.display_image(rotated_img)

    def negative_image(self):
        if self.image is not None:
            negative_img = cv2.bitwise_not(self.image)
            self.display_image(negative_img)

    def draw_circle(self):
        if self.image is not None:
            height, width = self.image.shape[:2]
            x, ok = QInputDialog.getInt(self, "Нарисовать круг", "Координата X:", 0, 0, width, 1)
            if ok:
                y, ok = QInputDialog.getInt(self, "Нарисовать круг", "Координата Y:", 0, 0, height, 1)
                if ok:
                    max_radius = min(x, y, width - x, height - y)
                    radius, ok = QInputDialog.getInt(self, "Нарисовать круг", f"Радиус (макс {max_radius}):", 0, 1, max_radius, 1)
                    if ok:
                        circled_img = self.image.copy()
                        cv2.circle(circled_img, (x, y),
                                   radius, (0, 0, 255), -1)
                        self.display_image(circled_img)

    def display_image(self, img):
        qformat = QImage.Format_Indexed8
        if len(img.shape) == 3:  # rows[0], cols[1], channels[2]
            qformat = QImage.Format_RGB888
        out_image = QImage(img, img.shape[1],
                           img.shape[0], img.strides[0], qformat)
        out_image = out_image.rgbSwapped()

        self.image_label.setPixmap(QPixmap.fromImage(out_image))
        self.image_label.setAlignment(Qt.AlignCenter)


if __name__ == "__main__":
    app = QApplication(sys.argv)
    window = ImageProcessorApp()
    window.show()
    sys.exit(app.exec_())
